...
class FultonSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://iaspublicaccess.fultoncountyga.gov/Search/Disclaimer.aspx?FromUrl=../search/advancedsearch.aspx?mode=advanced",
    ...
    def parse_property_residential_page(self, response):
        ...
        item["building_area_sqft"] = (
            response.xpath(
                '//tr[./td[text()="Total Living Area:"]]/td[@class="DataletData"]/text()'
            )
            .get("")
            .replace(",", "")
        )
        ...
        value_history_link = response.xpath(
            '//a[./span[text()="Values History"]]/@href'
        ).get()
        yield response.follow(
            value_history_link,
            meta={"item": item},
    ...
    def parse_property_sales_page(self, response):
        ...
        sale_date_str = response.xpath(
            '//tr[./td[text()="Sales Date:"]]/td[@class="DataletData"]/text()'
        ).get()
        ...
        item["sale_price"] = (
            response.xpath(
                '//table[@id="Sales"]//td[@bgcolor="#ffffc9"][last()]/text()'
            )
            .get("")
            .replace("$", "")
            .replace(",", "")
        ...
        item["transfer_deed_type"] = (
            response.xpath(
                '//tr[./td[text()="Deed Type:"]]/td[@class="DataletData"]/text()'
            )
            .get("")
            .replace("\xa0", "")
        )
