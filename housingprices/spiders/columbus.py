...
class ColumbusSpider(scrapy.Spider):
    name = 'columbus'
    allowed_domains = ['publicaccess.columbusga.org']
    start_urls = ['https://publicaccess.columbusga.org/Search/Disclaimer.aspx?FromUrl=../search/advancedsearch.aspx?mode=advanced',
            'https://publicaccess.columbusga.org/search/advancedsearch.aspx?mode=advanced']
    start_year = 1907
    state = "GA"
    county = "COLUMBUS"
    shards = []
    ...
    def parse_property_main_page(self, response):
        ...
        item['property_zip5'] = response.xpath('//td[./td[text()="Zip Code"]]/td[@class="DataletData"]/text()').get("")
        if len(item['property_zip5']) != 5 or item['property_zip5'] == '00000':
            item['property_zip5'] = None
        item['property_type'] = response.xpath('//tr[./td[text()="Class (Not Zoning)"]]/td[@class="DataletData"]/text()').get()
    ...
    def parse_property_residential_page(self, response):
        ...
        land_link = response.xpath('//a[./span[text()="Land Information"]]/@href').get()
    ...
    def parse_property_land_page(self, response):
        ...
        sales_link = response.xpath('//a[./span[text()="Sales Information"]]/@href').get()
    ...
    def parse_property_sales_page(self, response):
        ...
        action = response.xpath('//form[@name="frmMain"]/@action').get()
        form_url = urljoin(response.url, action).replace('mode=sales', 'mode=profileall')
