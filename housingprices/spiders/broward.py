...
class BrowardSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        while d < date.today():
            ...
            payload = 'type: "RES",subtype: "RESALL",datemin: "{}",datemax: "{}",amountmin: "",amountmax: "",beds: "0",baths: "0",qualified: "Y",minsale: "Y",location: "",order: "FOLIO",pagenum: "1",pagecount: "100"'.format(
                date_str, date_str
            )
    ...
    def parse_search_api_response(self, response):
        ...
        for result_dict in results:
            ...
            yield scrapy.Request(
                ...
                method="POST",
                body=payload,
                callback=self.parse_parcelinfo_api_response,
                meta={"tax_year": tax_year},
            )
        ...
        if len(results) == PER_PAGE:
            ...
            url = (
                "https://web.bcpa.net/bcpaclient/searchallsales.aspx/getSaleTypeResult"
            )
            yield scrapy.Request(
                url,
                method="POST",
    ...
    def parse_date_str(self, date_str):
        ...
        return date(
            year=int(components[2]), month=int(components[0]), day=int(components[1])
        )
    ...
    def parse_parcelinfo_api_response(self, response):
        ...
        while True:
            ...
            if type_field_value == "Square Foot":
                ...
                if type(fact_field_value) == str:
                    item["land_area_sqft"] = fact_field_value.replace(",", "").replace(
                        " SqFt", ""
                    )
            i += 1
        ...
        if json_dict.get("landValue") is not None:
            item["land_appraised_value"] = (
                json_dict.get("landValue").replace("$", "").replace(",", "")
            )
        if json_dict.get("bldgValue") is not None:
            item["building_appraised_value"] = (
                json_dict.get("bldgValue").replace("$", "").replace(",", "")
            )
        if json_dict.get("justValue") is not None:
            item["total_appraised_value"] = (
                json_dict.get("justValue").replace("$", "").replace(",", "")
            )
        ...
        while True:
            ...
            if type(item["sale_price"]) == str:
                item["sale_price"] = (
                    item["sale_price"].replace("$", "").replace(",", "")
                )
